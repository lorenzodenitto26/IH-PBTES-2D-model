...
Charge/discharge cycle model for the SAME ELECTRICAL ENERGY INPUT comparison.
...
Main idea:
    1. Charge until the same cumulative electrical energy input is spent.
...
def _compute_charging_powers_same_energy(
    self,
    freq,
    I,
    q_src,
    ...
    P_input:
        power used to integrate the cumulative electrical energy target [W]
...
# ============================================================
# CHARGING UNTIL SAME ELECTRICAL ENERGY INPUT
# ============================================================

def charging_2D_until_same_energy_from_map(
    self,
    freq,
    I,
    T_init_full,
    ...
    Stop criterion:
        cumulative electrical input energy reaches E_input_target_kWh.
    ...
    The electrical input used for the stopping criterion can be:
        electrical_input = "coil"
    ...
    """
    if E_input_target_kWh <= 0:
        raise ValueError("E_input_target_kWh must be positive.")
    ...
    E_target_J = float(E_input_target_kWh) * J_PER_KWH
    ...
    P_ind, P_coil, P_conv, P_input = self._compute_charging_powers_same_energy(
        freq=freq,
        I=I,
        q_src=q_src,
        T_coil=T_coil,
    ...
    if verbose:
        print("========== CHARGING: SAME ENERGY INPUT ==========")
        print(f"source_mode             = {source_mode}")
        print(f"freq                    = {freq:.3f} Hz")
        print(f"I                       = {I:.3f} A")
        print(f"electrical_input        = {electrical_input}")
        print(f"E_input_target          = {E_input_target_kWh:.3f} kWh")
    ...
    for n in range(n_steps_max):
        ...
        # Reduce last step to reach the requested energy almost exactly
        E_remaining = E_target_J - E_input_hist[-1]
        dt_eff = min(float(dt), E_remaining / P_input)
        ...
        E_ind_new = E_ind_hist[-1] + P_ind * dt_eff
        E_coil_new = E_coil_hist[-1] + P_coil * dt_eff
        E_conv_new = E_conv_hist[-1] + P_conv * dt_eff
        E_input_new = E_input_hist[-1] + P_input * dt_eff
        ...
        if ((n + 1) % save_every == 0) or (E_input_new >= E_target_J):
            snapshots.append(T.copy())
            snapshot_times.append(t_new)

        if Tmax_safety_K is not None and np.max(T) >= Tmax_safety_K:
            stop_reason = "Tmax_safety_reached"
            break

        if E_input_new >= E_target_J * (1.0 - 1e-10):
            stop_reason = "E_input_target_reached"
            break
    ...
    result = {
        ...
        "source_mode": source_mode,
        "electrical_input": electrical_input,
        "E_input_target_kWh": E_input_target_kWh,
...
def run_same_energy_input_cycles(
    self,
    case_name,
    freq,
    I,
    ...
    """
    Run repeated charge/discharge cycles with the same cumulative electrical
    input energy during each charging phase.

    For each cycle:
        1. charge until E_input_target_kWh is reached
    ...
    for cyc in range(1, n_cycles + 1):
        ...
        charge = self.charging_2D_until_same_energy_from_map(
            freq=freq,
            I=I,
...
def run_same_energy_input_comparison(
    self,
    cases,
    E_input_target_kWh,
    n_cycles=3,
    **common_kwargs,
    ...
    """
    Run the same-energy-input cycle analysis for multiple cases.
    ...
    for case in cases:
        res = self.run_same_energy_input_cycles(
...
def attach_same_energy_cycle_model(IHTES2D_Tdep):
    """
    Attach the same-energy-input cycling model to IHTES2D_Tdep.
    ...
    IHTES2D_Tdep._compute_charging_powers_same_energy = _compute_charging_powers_same_energy

    IHTES2D_Tdep.charging_2D_until_same_energy_from_map = charging_2D_until_same_energy_from_map
    IHTES2D_Tdep.discharging_2D_air_until_Tout = discharging_2D_air_until_Tout

    IHTES2D_Tdep.run_same_energy_input_cycles = run_same_energy_input_cycles
    IHTES2D_Tdep.run_same_energy_input_comparison = run_same_energy_input_comparison

    print("Same-energy-input cycle model attached to IHTES2D_Tdep.")
